...
{
    public class NextItemNoManager
    {
        public static Int32 GetNextItemNo()
        {
            using (DataContext db = new DataContext())
            {
                var number = db.NextItemNo.Select(m => m.kNextItemNo);
        ...
        public static void IncreaseNextItemNo()
        {
            using (DataContext db = new DataContext())
            {
                var number = db.NextItemNo.Select(m => m.kNextItemNo);
                var next_number = number.SingleOrDefault() + 1;
                NextItemNo obj = new NextItemNo();
                obj.kNextItemNo = Convert.ToInt32(next_number);
                obj.dtLastMod = DateTime.Now;


                db.Database.ExecuteSqlCommand("IncreaseNextItemNo @ItemNo", new SqlParameter("ItemNo", obj.kNextItemNo));
